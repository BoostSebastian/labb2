...
public class ClientHandler implements Runnable {
	...
	private void readResponse() throws IOException, InterruptedException {
		...
		try {
			...
			// Testing a 500 error
			// CHANGE THIS test_500 to simulate a 500
			...
			if (requestHeader.split("\n")[0].contains("GET")) {
				...
				// small fix for handeling htm
				if(path.contains("html")){
					// do nothing
				} else if(path.contains("htm") && !checkURL(file)){
					path += "l";
					_302 = true;
				}
					...
					if(!path.toLowerCase().contains("google")){ 						// redirect
						...
						if(checkURL(file)){ 											// if url exist or if we need to redirect
							...
							if (type == "html"){
								...
								if(_302){
									constructResponseHeader(302, sb, path);				// build header
								} else {
	...
	private static void constructResponseHeader(int responseCode, StringBuilder sb, String fileType) {
		...
		if (responseCode == 200) {
			...
			String link = "https://www.google.se/";
			if(fileType != ""){
				link = fileType;
			}

			sb.append("HTTP/1.1 302 Redirected\r\n");
			sb.append("Location: " + link + " \r\n");
			sb.append("Date:" + getTimeStamp() + "\r\n");
			sb.append("Server:localhost\r\n");
			sb.append("\r\n");
	...
	private static String buildURL(String file){
		...
		String _url = my_url.toString();
		if(_url.contains("html")){

		} else if(_url.contains("htm")) {
			_url += "l";
		}
